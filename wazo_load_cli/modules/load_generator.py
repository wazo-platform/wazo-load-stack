...
class LoadGenerator:
    def __init__(self, load_config: str, load_file: str, timer: Timer) -> None:
        ...
        self.disable_chatd = int(self.config.get("DEFAULT", "DISABLE_CHATD", fallback=1))
        self.duration = int(self.config.get("DEFAULT", "DURATION", fallback=300))
        self.token_expiration = int(self.config.get("DEFAULT", "TOKEN_EXPIRATION", fallback=600))
        self.delay_cnx_rand = int(self.config.get("DEFAULT", "DELAY_CNX_RAND", fallback=60))
        self.ttl = int(self.config.get("DEFAULT", "TTL", fallback=30))
        self.server = self.config.get("DEFAULT", "SERVER", fallback="wazo-5000-1.load.wazo.io")
        self.load_sections = int(self.config.get("DEFAULT", "LOAD_SECTIONS", fallback=10))
        ...
        self.request_timeout = int(self.config.get("DEFAULT", "REQUEST_TIMEOUT", fallback=300))
        self.disable_header_check = int(self.config.get("DEFAULT", "DISABLE_HEADER_CHECK", fallback=1))
        self.trafgen_number = int(self.config.get("DEFAULT", "TRAFGEN_NUMBER", fallback=10))
        self.load_files_number = int(self.config.get("DEFAULT", "LOAD_FILES_NUMBER", fallback=1))
    ...
    def generate_load_section(self, file: str) -> None:
        host_num = 0
        for _ in range(self.trafgen_number):
            ...
            for _ in range(self.clients):
                ...
                file.write(f'      cmd: "sleep {timer} && node /usr/src/app/index.js"\n')
                file.write(f"      env:\n")
                file.write(f"        LOGIN: {self.start_user}{self.ext}\n")
                file.write(f"        PASSWORD: superpass\n")
                ...
                file.write(f"        DISABLE_HEADER_CHECK: {self.disable_header_check}\n")
...
if __name__ == "__main__":
    ...
    load = LoadGenerator("/home/user/src/wls/wazo_load_cli/load.ini", "wda", randomized_timer)
